...
def test_slope_perfect_linear(simple_ts):
    ...
    # Perfect linear data → p_value should be very small (significant trend)
    assert a["p_value"] < 0.05
    assert b["p_value"] < 0.05
    # Perfect linear data → std_err should be near zero
    assert a["std_err"] < 0.01
    assert b["std_err"] < 0.01
...
def test_slope_single_point():
    ...
    # p_value and std_err should also be NaN when slope can't be computed
    assert np.isnan(result.iloc[0]["p_value"])
    assert np.isnan(result.iloc[0]["std_err"])


def test_slope_output_columns(simple_ts):
    result = compute_trend_slope(simple_ts)
...
def test_rolling_avg_center_window():
    ts = pd.DataFrame([
        {"region": "A", "year": y, "prevalence_pct": float(
            y), "measure": "obesity"}
...
def test_rolling_avg_multiple_regions():
    ...
    a_mid = result[(result["region"] == "A") &
                   (result["year"] == 2012)].iloc[0]
    b_mid = result[(result["region"] == "B") &
                   (result["year"] == 2012)].iloc[0]
...
def test_prevalence_weighted_average():
    ...
    df = pd.DataFrame([
        {"year": 2020, "region": "A", "measure": "obesity",
            "value": 10.0, "sample_size": 100},
        {"year": 2020, "region": "A", "measure": "obesity",
            "value": 20.0, "sample_size": 100},
    ])
